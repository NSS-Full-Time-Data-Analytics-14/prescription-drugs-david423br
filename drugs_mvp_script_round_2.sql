...
-- 1a ANSWER: NPI# 1881634483 with 99707 claims.
...
GROUP BY specialty_description
ORDER BY sum_opioid_claim_count DESC;
...
-- 2c ANSWER: 15 specialties do not show in the prescription table.
...
SELECT specialty_description,
	   ...
	   opioid_drug_flag
FROM (SELECT specialty_description,
	   total_claim_count,
	   opioid_drug_flag
FROM prescription
INNER JOIN drug
...
INNER JOIN prescriber
USING(npi)
GROUP BY specialty_description, total_claim_count, opioid_drug_flag)
GROUP BY specialty_description, opioid_drug_flag
ORDER BY specialty_description;

-- 2d ANSWER: Top 5: Case Manager/Care Coordinator, Orthopaedic Surgery, Interventional Pain Management, Pain Management, Anesthesiology

-- 3.	a. Which drug (generic_name) had the highest total drug cost?

SELECT
	generic_name,
	SUM(total_drug_cost) AS sum_drug_cost
FROM drug
INNER JOIN prescription
	USING(drug_name)
...
-- 3b ANSWER: C1 ESTERASE INHIBITOR - 3495.22
...
SELECT
	COUNT(cbsa) AS cbsa_tn_count
FROM cbsa
WHERE cbsaname LIKE ('%TN');

-- 5a ANSWER: 33 CBSAs in TN
...
-- 5b ANSWER: Largest: "Nashville-Davidson--Murfreesboro--Franklin, TN" with 1830410. Smallest: "Morristown, TN" with 116352.
